...
export default class IcalCalendar extends Homey.App {
  ...
  getSyncInterval(): void {
    ...
    const syncInterval: SyncInterval | null =
      (this.homey.settings.get(variableMgmt.setting.syncInterval) as SyncInterval) || null;
    if (syncInterval) {
    ...
    this.homey.settings.on("set", (args: string): void => {
      ...
      if (
        [
          ...
          variableMgmt.setting.eventLimit,
          variableMgmt.setting.nextEventTokensPerCalendar
        ].includes(args)
      ) {
        // sync calendars when calendar specific settings have been changed
        if (variableMgmt.gettingEventsRunning) {
          this.log(
            `registerSettingCallbacks/${args}: "getEvents" is currently running. Updated settings won't be applied until the next 15th minute!`
          );
          return;
        }
      ...
      if (
        [
          variableMgmt.setting.dateFormatLong,
          variableMgmt.setting.dateFormatShort,
          variableMgmt.setting.timeFormat
        ].includes(args)
      ) {
  ...
  async startJobs(type?: string): Promise<void> {
    const updateFunc = (): void => {
      ...
      if (
        variableMgmt.gettingEventsLastRun &&
        (Date.now() - variableMgmt.gettingEventsLastRun.getTime()) / 1000 / 60 < 5
      ) {
        this.log(
          "[WARN] startJobs/update: Wont update calendars from this job since there's less than 5 minutes since getEvents was last executed:",
          variableMgmt.gettingEventsLastRun
        );
        return;
      }
    ...
    if (!type) {
      ...
      variableMgmt.jobs.trigger = addJob("*/1 * * * *", async (): Promise<void> => {
        ...
        if (variableMgmt.gettingEventsRunning) {
          this.log(
            "[WARN] startJobs/trigger: Wont update tokens and trigger events since getEvents is running. Will update/trigger in one minute"
          );
          return;
        }
      ...
      if (!isValidCron(interval.cron)) {
        this.error(
          `[ERROR] startJobs: Auto update will be disabled. Invalid cron value specified in settings: '${interval.cron}'`
        );
        try {
          await triggerSynchronizationError({
            app: this,
            variableMgmt,
            calendar: "cron syntax",
      ...
      this.log(
        `startJobs: Auto update enabled with cron value '${interval.cron}'. Next update in UTC: ${variableMgmt.jobs.update.nextRun()}`
      );
    ...
    if (type === "update") {
      ...
      if (!isValidCron(interval.cron)) {
        this.error(
          `[ERROR] startJobs(update): Auto update will be disabled. Invalid cron value specified in settings: '${interval.cron}'`
        );
        delete variableMgmt.jobs.update;
        try {
          await triggerSynchronizationError({
      ...
      this.log(
        `startJobs(update): Auto update enabled with cron value '${interval.cron}'. Next update in UTC: ${variableMgmt.jobs.update.nextRun()}`
      );
    }
  }
