import { App, FlowCard, FlowCardAction } from "homey";
...
import { VariableManagement, VariableManagementCalendar, VariableManagementCalendarEvent, VariableManagementLocalEvent } from "../types/VariableMgmt.type";

const getDateTime = (value: string): string | null => {
  const match: RegExpExecArray | null = /[1-2][0-9][0-9][0-9]-((0[1-9])|(1[0-2]))-((0[1-9])|(1[0-9])|(2[0-9])|(3[0-1]))T([0-1][0-9]|2[0-3]):[0-5][0-9]:[0-5][0-9]Z?/g.exec(value);
...
export const setupActions = (app: App, variableMgmt: VariableManagement): void => {
  ...
  app.homey.flow.getActionCard('sync-calendar').registerRunListener(async (_, __) => {
    app.log(`sync-calendar: Action card triggered. ${variableMgmt.gettingEventsRunning ? 'getEvents already running' : 'Triggering getEvents without reregistering tokens'}`);
  ...
  newEventAction.registerRunListener(async (args, _) => {
    ...
    const calendar: VariableManagementCalendar | undefined = variableMgmt.calendars.find((c: VariableManagementCalendar) => c.name === event.calendar);
    if (!calendar) {
    ...
    await triggerEvents(app, variableMgmt, app.homey.clock.getTimezone(), { calendarName: event.calendar, event, triggerId: 'event_added' });

    return true;
  });
  newEventAction.registerArgumentAutocompleteListener('calendar', (query: string, _): FlowCard.ArgumentAutocompleteResults => calendarAutocomplete(app, variableMgmt, query));
  ...
  app.homey.flow.getActionCard('delete_event_name').registerRunListener(async (args, _) => {
    ...
    const event: VariableManagementLocalEvent | undefined = Array.isArray(variableMgmt.localEvents) ? variableMgmt.localEvents.find((e: VariableManagementLocalEvent) => e.summary === args.event_name) : undefined;
    if (!event) {
    ...
    const newCalendarEvents: VariableManagementCalendarEvent[] = calendar.events.filter((e: VariableManagementCalendarEvent) => e.summary !== args.event_name);
    const removedEvents: number = calendar.events.length - newCalendarEvents.length;
    calendar.events = newCalendarEvents;
    variableMgmt.localEvents = variableMgmt.localEvents.filter((e: VariableManagementLocalEvent) => e.summary !== args.event_name);
  ...
  app.homey.flow.getActionCard('get_calendars_metadata').registerRunListener((_, __) => {
    ...
    return {
      json: JSON.stringify(variableMgmt.calendars.map((calendar: VariableManagementCalendar) => {
        return {
  ...
  getCalendarEvent.registerRunListener((args, _) => {
    ...
    const calendar: VariableManagementCalendar | undefined = variableMgmt.calendars.find((cal: VariableManagementCalendar) => cal.name === args.calendar.name);
    if (!calendar) {
    ...
    return {
      ...
      event_created: event.created ? event.created.format(`${variableMgmt.dateTimeFormat.long} ${variableMgmt.dateTimeFormat.time}`) : '',
  ...
  getCalendarEvent.registerArgumentAutocompleteListener('calendar', (query: string, _): FlowCard.ArgumentAutocompleteResults => calendarAutocomplete(app, variableMgmt, query));
}
