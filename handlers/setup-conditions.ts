import { App, FlowCard, FlowCardCondition } from "homey";
import { Moment } from "moment";

import { isEventOngoing, isEventIn, willEventNotIn } from './conditions.js';
...
import { filterByCalendar, filterBySummary, filterByUID, filterByProperty } from '../lib/filter-by.js';
...
import { ExtCalendarEvent, VariableManagement, VariableManagementCalendar, VariableManagementCalendarEvent } from "../types/VariableMgmt.type";
...
const getEventList = (app: App, variableMgmt: VariableManagement, timezone: string, calendars: VariableManagementCalendar[]): Array<EventAutoCompleteResult> => {
  ...
  calendars.forEach((calendar: VariableManagementCalendar) => {
    calendar.events.forEach((event: VariableManagementCalendarEvent) => {
      ...
      try {
        ...
        app.error(`[ERROR] getEventList: Failed to parse 'start' (${startMoment}) or 'end' (${endMoment}):`, error, event);
        startStamp = '';
        endStamp = '';

        triggerSynchronizationError({ app, variableMgmt, calendar: calendar.name, error: error as Error | string, event })
...
const onEventAutocomplete = async (app: App, variableMgmt: VariableManagement, timezone: string, query: string, type: ConditionCardArgumentId): Promise<Array<EventAutoCompleteResult>> => {
...
const checkEvent = async (app: App, variableMgmt: VariableManagement, timezone: string, args: any, type: ConditionCardRunListenerId): Promise<boolean> => {
  ...
  if (['any_in_calendar', 'any_ongoing_calendar', 'event_containing_calendar', 'event_containing_calendar_stops', 'equal_in', 'match_in'].includes(type)) {
  ...
  if (type === 'event_containing_calendar') {
    ...
    const nextEvent: ExtCalendarEvent | null = getNextEventValue({ calendars: filteredCalendars, specificCalendarName: args.calendar.name, value: args.value, eventType: 'starts', timezone });
    if (!nextEvent) {
    ...
    app.log(`checkEvent/event_containing_calendar: Next event containing found: '${nextEvent.summary}' ${(nextEvent.start)}. Starts within ${inMinutes} minutes? ${startsWithin}`);
    if (startsWithin) {
  ...
  if (type === 'event_containing_calendar_stops') {
    const inMinutes: number = convertToMinutes(args.when, args.type);
    const nextEvent: ExtCalendarEvent | null = getNextEventValue({ calendars: filteredCalendars, specificCalendarName: args.calendar.name, value: args.value, eventType: 'ends', timezone });
    if (!nextEvent) {
    ...
    app.log(`checkEvent/event_containing_calendar_stops: Next event containing found: '${nextEvent.summary}' ${(nextEvent.end)}. Ends within ${inMinutes} minutes? ${endsWithin}`);
    if (endsWithin) {
      await updateNextEventWithTokens(app, variableMgmt, nextEvent);
    }
    return endsWithin;
  ...
  if (type === 'equal_in') {
    ...
    if (args.when === undefined || args.type === undefined || args.when === null || args.type === null) {
      app.log(`checkEvent: Equal_in found ${filteredCalendars[0].events.length} events in calendar ${filteredCalendars[0].name} matching '${args.property}' with '${args.value}'. No timeframe given (when:'${args.when}', type:'${args.type}')! Returning true`);
      return true;
    }
  }
  ...
  if (type === 'match_in') {
    ...
    if (args.when === undefined || args.type === undefined || args.when === null || args.type === null) {
      app.log(`checkEvent: Match_in found ${filteredCalendars[0].events.length} events in calendar ${filteredCalendars[0].name} where '${args.property}' '${args.matcher}' '${args.value}'. No timeframe given (when:'${args.when}', type:'${args.type}')! Returning true`);
      return true;
    }
  }
  ...
  cards.forEach(({ id, runListenerId, autocompleteListener }: ConditionCard) => {
    ...
    if (autocompleteListener.argumentId && autocompleteListener.id) {
      if (autocompleteListener.id === 'calendar') {
        conditionCard.registerArgumentAutocompleteListener(autocompleteListener.argumentId, (query: string, _): FlowCard.ArgumentAutocompleteResults => calendarAutocomplete(app, variableMgmt, query));
        return;
      }

      conditionCard.registerArgumentAutocompleteListener(autocompleteListener.argumentId, async (query: string, _): Promise<FlowCard.ArgumentAutocompleteResults> => onEventAutocomplete(app, variableMgmt, timezone, query, autocompleteListener.id));
    }
  });
}
