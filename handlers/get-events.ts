...
import { generateTokens, generatePerCalendarTokens, generateNextEventTokens } from '../lib/generate-token-configuration.js';
...
import { CalendarEventUid, CalendarMetaData, IcalSettingEntry } from "../types/IcalCalendar.type";
import {
  ExtCalendarEvent,
  SettingEventLimit,
  VariableManagement,
...
export const getEvents = async (app: App, variableMgmt: VariableManagement, reregisterCalendarTokens: boolean = false): Promise<string[]> => {
  if (!variableMgmt) {
  ...
  const eventLimit: SettingEventLimit = app.homey.settings.get(variableMgmt.setting.eventLimit) as SettingEventLimit || variableMgmt.setting.eventLimitDefault;
  const oldCalendarsUidsStorage: string | null = app.homey.settings.get(variableMgmt.storage.eventUids) as string || null;
  const oldCalendarsUids: CalendarEventUid[] = oldCalendarsUidsStorage && oldCalendarsUidsStorage.length > 0 ? JSON.parse(oldCalendarsUidsStorage) as CalendarEventUid[] : [];
  ...
  for (let i: number = 0; i < calendars.length; i++) {
    ...
    try {
      ...
      app.error(`[ERROR] getEvents: Failed to get events for calendar '${name}' with uri '${uri}' :`, errorString || error);
      try {
        app.log(`[WARN] getEvents: Getting events (${eventLimit.value} ${eventLimit.type} ahead) for calendar`, name, 'with fallback uri', fallbackUri);
        data = await iCal.fromURL(uri);
      } catch (innerError) {
        const fallbackErrorString: string | undefined = innerError instanceof Error ? `${innerError.message} -> ${innerError.stack}` : undefined;
        app.error(`[ERROR] getEvents: Failed to get events for calendar '${name}' with fallback uri '${fallbackUri}' :`, fallbackErrorString || innerError);
    ...
    if (data) {
      ...
      try {
        app.log(`getEvents: Events for calendar '${name}' retrieved. Total entry count for calendar: ${Object.keys(data).length}. Time used: ${getWorkTime(retrieveCalendarStart, retrieveCalendarEnd)}`);
        const activeEvents: VariableManagementCalendarEvent[] = getActiveEvents({ app, timezone: app.homey.clock.getTimezone(), data, eventLimit, calendarName: name, logAllEvents });
        app.log(`getEvents: Active events for calendar '${name}' updated. Event count: ${activeEvents.length}. Time used: ${getWorkTime(retrieveCalendarEnd, new Date())}`);
        calendarsEvents.push({ name, events: activeEvents });
        calendarsMetadata.push({ name, eventCount: activeEvents.length, lastSuccessfullSync: getMoment({ timezone: app.homey.clock.getTimezone() }) });
      } catch (error) {
        const errorString: string | undefined = error instanceof Error ? `${error.message} -> ${error.stack}` : undefined;
        app.error(`[ERROR] getEvents: Failed to get active events for calendar '${name}'. Time used: ${getWorkTime(retrieveCalendarEnd, new Date())} :`, errorString || error);
        ...
        calendarsMetadata.push({ name, eventCount: 0, lastFailedSync: getMoment({ timezone: app.homey.clock.getTimezone() }) });
      ...
      app.log(`[WARN] getEvents: Calendar '${name}' not reachable! Giving up... Time used: ${getWorkTime(retrieveCalendarStart, new Date())}`);
    }
  }

  try {
    if (variableMgmt.calendars && variableMgmt.calendars.length > 0 && calendarsEvents.length > 0) {
      const updatedCalendars: VariableManagementCalendar[] = filterUpdatedCalendars({ app, variableMgmt, oldCalendars: variableMgmt.calendars, newCalendars: calendarsEvents });
    ...
    try {
      await triggerSynchronizationError({ app, variableMgmt, calendar: 'Changed calendars', error: error as Error | string });
    } catch (triggerError) {
  ...
  const newlyAddedEvents: ExtCalendarEvent[] = getNewEvents({ timezone: app.homey.clock.getTimezone(), oldCalendarsUids, newCalendarsUids, calendarsEvents, app });
  app.log('getEvents: newlyAddedEvents --', newlyAddedEvents.length);
  for await (const event of newlyAddedEvents) {
    await triggerEvents(app, variableMgmt, app.homey.clock.getTimezone(), { calendarName: event.calendarName, event, triggerId: 'event_added' });
    await triggerEvents(app, variableMgmt, app.homey.clock.getTimezone(), { calendarName: event.calendarName, event, triggerId: 'event_added_calendar', state: { calendarName: event.calendarName } });
  }
  ...
  const localEvents: VariableManagementLocalJsonEvent[] = localEventsJSON && localEventsJSON.length > 0 ? JSON.parse(localEventsJSON) as VariableManagementLocalJsonEvent[] : [];
  variableMgmt.localEvents = getLocalActiveEvents({ timezone: app.homey.clock.getTimezone(), events: localEvents, eventLimit, app, logAllEvents });
  ...
  variableMgmt.localEvents.forEach((event: VariableManagementLocalEvent) => {
    const calendar: VariableManagementCalendar | undefined = calendarsEvents.find((c: VariableManagementCalendar) => c.name === event.calendar);
  ...
  app.log(`getEvents: All events count: ${allEventCount}. Time used: ${getWorkTime(retrieveCalendarsStart, new Date())}`);
  ...
  if (reregisterCalendarTokens) {
    ...
    if (variableMgmt.calendarTokens.length > 0) {
      ...
      await Promise.all(variableMgmt.calendarTokens.map(async (tokenId: string) => {
        try {
    ...
    if (variableMgmt.nextEventWithTokens.length > 0) {
      ...
      await Promise.all(variableMgmt.nextEventWithTokens.map(async (tokenId: string) => {
        try {
          const token: FlowToken = app.homey.flow.getToken(tokenId);
          if (token) {
    ...
    const nextEventTokensPerCalendar: boolean = app.homey.settings.get(variableMgmt.setting.nextEventTokensPerCalendar) as boolean || false;

    // register calendar tokens
    if (variableMgmt.calendars && variableMgmt.calendars.length > 0) {
      await Promise.all(variableMgmt.calendars.map(async (calendar: VariableManagementCalendar) => {
        if (!variableMgmt) {
          app.error(`[ERROR] getEvents: Variable management initialization failed. Calendar '${calendar.name}' won't be used!`);
          return Promise.resolve();
        }

        // register today's and tomorrow's events pr calendar
        generateTokens(app, variableMgmt, calendar.name).map(async ({ id, type, title }: { id: string, type: string, title: string }) => {
          try {
            if (!variableMgmt) {
              app.error(`[ERROR] getEvents: Variable management initialization failed. Calendar token '${id}' won't be created!`);
              return Promise.resolve();
            }
        ...
        if (nextEventTokensPerCalendar) {
          generatePerCalendarTokens(app, variableMgmt, calendar.name).map(async ({ id, type, title }: { id: string, type: string, title: string }) => {
            try {
              if (!variableMgmt) {
                app.error(`[ERROR] getEvents: Variable management initialization failed. Per calendar token '${id}' won't be created!`);
                return Promise.resolve();
              }
