...
import { CalendarMetaData, IcalSettingEntry } from "../../types/IcalCalendar.type";
...
export class MyDevice extends Device {
  ...
  async onInit (): Promise<void> {
    this.log(`Device v${this.homey.manifest.version} is running on firmware ${this.homey.version} with Timezone: '${this.homey.clock.getTimezone()}'`);
  ...
  async updateCalendarsCount (): Promise<void> {
    const calendarsConfigured: IcalSettingEntry[] = this.homey.settings.get(variableMgmt.setting.icalUris) as IcalSettingEntry[];
    ...
    const currentCalendarCapabilities: string[] = this.getCapabilities().filter((capability: string) => capability.includes(lastSuccessfulSync) || capability.includes(eventCountPerCalendar)).map((capability: string) => capability.replace(`${lastSuccessfulSync}.`, '').replace(`${eventCountPerCalendar}.`, ''));
    ...
    for await (const calendarName of currentCalendarNames) {
      if (!calendarsConfigured.find((calendar: IcalSettingEntry) => calendar.name === calendarName)) {
        this.log('[WARN] updateCalendarsCount -', calendarName, 'is no longer a configured calendar but has still registered capabilities. Removing capabilities for this calendar');
        try {
          ...
          this.error('[ERROR] updateCalendarsCount - Failed to remove capabilities for calendar no longer configured:', ex);
        }
      }
    }
  }
  ...
  async updateCalendarsMetadata (): Promise<void> {
    ...
    const calendarsMetadata: CalendarMetaData[] | undefined = calendarsMetadataStr ? JSON.parse(calendarsMetadataStr) : undefined;
    ...
    for await (const calendar of calendarsMetadata) {
      ...
      if (this.hasCapability(lastSuccessfulSyncLoop)) {
        await this.updateCapabilityValue(lastSuccessfulSyncLoop, getMoment({ timezone: this.homey.clock.getTimezone(), date: calendar.lastSuccessfullSync?.toISOString() || '01.01.1970 00:00:00' }).format('DD.MM.YYYY HH:mm:ss'));
      } else {
   ...
   */
  async onSettings ({ oldSettings, newSettings, changedKeys }: { oldSettings: any, newSettings: any, changedKeys: string[] }): Promise<void> {
    this.log('MyDevice settings where changed. Changed keys:', changedKeys, 'New settings:', newSettings, 'Old settings:', oldSettings);
  }
