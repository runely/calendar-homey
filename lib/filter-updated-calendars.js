...
const isChanged = (app, previous, current) => {
  ...
  if (hasData(previous) && !hasData(current)) {
    app.warn(`filterUpdatedCalendars: Previous value had data but current value does not. This is probably a sync hickup -- Previous: '${previous}' , Current: '${current}'`)
    return false
  }
  if (!hasData(previous) && hasData(current)) {
    app.warn(`filterUpdatedCalendars: Previous value did not have data but current value does. This is probably a sync hickup -- Previous: '${previous}' , Current: '${current}'`)
    return false
  }
  return false
}
