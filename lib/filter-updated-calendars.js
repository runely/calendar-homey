...
module.exports = options => {
  ...
  newCalendars.forEach(newCalendar => {
    ...
    newCalendar.events.forEach(newEvent => {
      ...
      if (isChanged(oldEvent.start, newEvent.start)) changed.push({ type: app.homey.__('triggers.event_changed.start'), previousValue: `${oldEvent.start.format(app.variableMgmt.dateTimeFormat.date.long)} ${oldEvent.start.format(app.variableMgmt.dateTimeFormat.time.time)}`, newValue: `${newEvent.start.format(app.variableMgmt.dateTimeFormat.date.long)} ${newEvent.start.format(app.variableMgmt.dateTimeFormat.time.time)}` })
      if (isChanged(oldEvent.end, newEvent.end)) changed.push({ type: app.homey.__('triggers.event_changed.end'), previousValue: `${oldEvent.end.format(app.variableMgmt.dateTimeFormat.date.long)} ${oldEvent.end.format(app.variableMgmt.dateTimeFormat.time.time)}`, newValue: `${newEvent.end.format(app.variableMgmt.dateTimeFormat.date.long)} ${newEvent.end.format(app.variableMgmt.dateTimeFormat.time.time)}` })
