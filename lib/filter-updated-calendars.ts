...
import { FilterUpdatedCalendarsOptions } from "../types/Options.type";
import { VariableManagementCalendar, VariableManagementCalendarEvent } from "../types/VariableMgmt.type";
import { CalendarPropertyChanged } from "../types/IcalCalendar.type";
...
function isChanged<T>(app: App, previous: T, current: T): boolean {
  if (hasData<T>(previous) && hasData<T>(current)) {
    ...
    app.log(`[WARN] filterUpdatedCalendars/isChanged: Previous and current has values, but are not moment or string types -- Previous: '${previous}' (${typeof previous}) , Current: '${current}' (${typeof current}). isChanged should be updated to handle this type properly.`);
    return false;
  }

  if (hasData<T>(previous) && !hasData<T>(current)) {
    app.log(`[WARN] filterUpdatedCalendars: Previous value had data but current value does not. This is probably a sync hiccup -- Previous: '${previous}' (${typeof previous}) , Current: '${current}' (${typeof current})`);
    return false;
  }

  if (!hasData<T>(previous) && hasData<T>(current)) {
    app.log(`[WARN] filterUpdatedCalendars: Previous value did not have data but current value does. This is probably a sync hiccup -- Previous: '${previous}' (${typeof previous}) , Current: '${current}' (${typeof current})`);
    return false;
  }

  app.error(`[ERROR] filterUpdatedCalendars/isChanged: Both previous and current values are missing data. This should not happen -- Previous: '${previous}' (${typeof previous}) , Current: '${current}' (${typeof current})`);
  return false;
}
...
export const filterUpdatedCalendars = (options: FilterUpdatedCalendarsOptions): VariableManagementCalendar[] => {
  ...
  newCalendars.forEach((newCalendar: VariableManagementCalendar) => {
    ...
    const oldCalendar: VariableManagementCalendar | undefined = oldCalendars.find((calendar: VariableManagementCalendar) => calendar.name === newCalendarName);
    ...
    newCalendar.events.forEach((newEvent: VariableManagementCalendarEvent) => {
      const oldEvent: VariableManagementCalendarEvent | undefined = oldCalendarEvents.find((event: VariableManagementCalendarEvent) => event.uid === newEvent.uid);
      if (!oldEvent) {
        return;
      }
      ...
      if (variableMgmt.dateTimeFormat?.long === undefined || variableMgmt.dateTimeFormat?.time === undefined) {
        app.error(`[ERROR] filterUpdatedCalendars: dateTimeFormat is not properly defined in variableMgmt. long: '${variableMgmt.dateTimeFormat?.long}' , time: '${variableMgmt.dateTimeFormat?.time}'`);
      ...
      const updatedCalendar: VariableManagementCalendar | undefined = updatedCalendars.find((calendar: VariableManagementCalendar) => calendar.name === newCalendarName);
