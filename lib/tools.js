...
module.exports.filterActiveEvents = (json) => {
    return (json.VEVENT || json).filter(event => isEventNotPast(event));
}

module.exports.sortEvents = (json) => {
...
module.exports.filterIcalBySummary = (json, summary) => {
    ...
    json.forEach(item => {
        (item.events.VEVENT || item.events).filter(event => event.SUMMARY.toLowerCase().includes(summary.toLowerCase()));
...
module.exports.filterIcalByUID = (json, uid) => {
    ...
    json.forEach(item => {
        (item.events.VEVENT || item.events).filter(event => event.UID.includes(uid));
        events.push({ ...item, events: itemEvents });
    });
