...
module.exports = options => {
  ...
  if (event.end) return event.skipTZ ? getMoment(event.end) : getMoment(event.end, timezone)
  if (event.datetype && event.datetype === 'date' && (!event.duration || typeof event.duration !== 'string')) return event.skipTZ ? getMoment(event.start).add(1, 'day') : getMoment(event.start, timezone).add(1, 'day')
  if (event.datetype && event.datetype === 'date-time') return event.skipTZ ? getMoment(event.start) : getMoment(event.start, timezone)
