...
import { VariableManagement, VariableManagementDateTimeFormat } from "../types/VariableMgmt.type";

const getShortDateFormat = (longDateFormat: string): string => {
  let dateFormat: string = longDateFormat;
  let dateLong: string = dateFormat;
  const legacyPatternResult: RegExpExecArray | null = /[d \-.,/:]{0,7}([DM]{1,2}[/.-][DM]{1,2}[/.-]Y{2,4})|[d \-.,/:]{0,7}(Y{2,4}[/.-][DM]{1,2}[/.-][DM]{1,2})/.exec(dateFormat);
...
export const getDateTimeFormat = (app: App, variableMgmt: VariableManagement): VariableManagementDateTimeFormat => {
  const time: string = app.homey.settings.get(variableMgmt.setting.timeFormat) || app.homey.__('settings.datetime.time.default');
  const long: string = app.homey.settings.get(variableMgmt.setting.dateFormatLong) || app.homey.settings.get(variableMgmt.setting.dateFormat) || app.homey.__('settings.datetime.date.default');
