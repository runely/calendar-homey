...
const updateHitCount = (app, id, args = undefined) => {
  ...
  if (args === undefined) {
    if (trigger.variants.length === 0) {
      ...
      const before = { ...trigger.variants[0] }
      ...
      app.log(`updateHitCount : Trigger variant for ${id} updated`, before, trigger.variants[0])
    }
  } else {
    ...
    if (!variant) {
      ...
      variant.today++
      variant.total++
      app.log(`updateHitCount : Trigger variant for ${id} updated`, before, variant)
    }
  }
