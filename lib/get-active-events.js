...
const convertToText = (app, prop, value, uid) => {
  if (typeof value === 'object') {
    app.warn(`getActiveEvents/convertToText - '${prop}' was object. Using 'val' of object '${uid}'`)
    return value.val
  }
  return value
}
...
module.exports = (options) => {
  ...
  let now = moment({ timezone })
  let eventLimitStart = moment({ timezone }).startOf('day')
  let eventLimitEnd = moment({ timezone }).endOf('day').add(Number.parseInt(eventLimit.value, 10), eventLimit.type)
  const events = []
  ...
  for (const event of Object.values(data)) {
    ...
    if (typeof event.rrule !== 'undefined') {
      try {
        ...
        //      Include dates which falls on until date: true||false
        let rruleStart = (event.rrule.options && event.rrule.options.dtstart && moment({ timezone: event.rrule.options.tzid || timezone, date: event.rrule.options.dtstart }).startOf('day').toDate()) || eventLimitStart.toDate()
        let rruleEnd = (event.rrule.options && event.rrule.options.until && moment({ timezone: event.rrule.options.tzid || timezone, date: event.rrule.options.until }).endOf('day').toDate()) || eventLimitEnd.toDate()
        ...
        let ongoingDates = event.rrule.between(rruleStart, rruleEnd, true).filter((date) => !moment(date).isAfter(eventLimitEnd.toDate()) && !dates.map((d) => d.getTime() === date.getTime()).includes(true))
        ...
        if (event.recurrences !== undefined) {
          for (const recurrenceDate of Object.keys(event.recurrences)) {
            ...
            let rMomentRecurr = recurrence.recurrenceid.toUTCString().includes('00:00:00') ? moment({ date: recurrence.recurrenceid }) : moment({ date: recurrence.recurrenceid, timezone })
            let rDateRecurr = new Date(getPaddedDateString(rMomentRecurr))
            let newDates = []
            ...
            let rMomentStart = recurrence.start.toUTCString().includes('00:00:00') ? moment({ date: recurrence.start }) : moment({ date: recurrence.start, timezone })
            let rDateStart = new Date(recurrence.start)
            ...
            if (rMomentStart.isBetween(eventLimitStart, eventLimitEnd) === true) {
              ...
              if (existAlready.length === 0) {
                ...
                app.log(`Recurrence in '${event.summary}' (${event.uid}) was moved from '${rMomentRecurr}' to ${rMomentStart}`)
              }
            } else {
              let rMomentEnd = recurrence.end.toUTCString().includes('00:00:00') ? moment({ date: recurrence.end }) : moment({ date: recurrence.end, timezone })
              if (rMomentStart.isBefore(eventLimitStart) && rMomentEnd.isAfter(eventLimitStart)) {
                ...
                if (existAlready.length === 0) {
                  ...
                  app.log(`Recurrence in '${event.summary}' (${event.uid}) was moved from '${rMomentRecurr}' to ${rMomentStart} AND is ongoing from '${rMomentStart}' to '${rMomentEnd}'`)
                }
              }
        ...
        localDates.forEach((date) => {
          ...
          let addEvent = true
          let offsetThis = moment({ timezone, date: event.start }).utcOffset()
          ...
          let dateLookupKey = (movedRecurrence !== undefined ? momentInstance(movedRecurrence.original) : momentInstance(start)).add(offsetThis, 'minutes').toISOString().slice(0, 10)
          ...
          let previousDay = (event.skipTZ ? (movedRecurrence !== undefined ? momentInstance(movedRecurrence.original) : momentInstance(start)) : (movedRecurrence !== undefined ? momentInstance(movedRecurrence.original) : momentInstance(start)).add(offsetThis, 'minutes')).subtract(1, 'day')
          let previousDayLookupKey = previousDay.toISOString().slice(0, 10)

          // for each date that we're checking, it's possible that there is a recurrence override for that one day.
          if (newEvent.recurrences !== undefined && newEvent.recurrences[dateLookupKey] !== undefined) {
            // we found an override, so for this recurrence, use a potentially different title, start date, and duration.
            let recdate = event.skipTZ ? moment({ date: newEvent.recurrences[dateLookupKey].start }) : moment({ timezone, date: newEvent.recurrences[dateLookupKey].start })
          ...
          if (newEvent.recurrences !== undefined && newEvent.recurrences[previousDayLookupKey] !== undefined) {
            ...
            let recdate = event.skipTZ ? moment({ date: newEvent.recurrences[previousDayLookupKey].start }) : moment({ timezone, date: newEvent.recurrences[previousDayLookupKey].start })
            if (recdate.isSame(start, 'day')) {
          ...
          if (end.isBefore(now) || start.isAfter(eventLimitEnd)) {
            ...
            if (newEvent.exdate[dateLookupKey] !== undefined) {
              ...
              let exdate = event.skipTZ ? moment({ date: newEvent.exdate[dateLookupKey] }) : moment({ timezone, date: newEvent.exdate[dateLookupKey] })
            ...
            if (newEvent.exdate[previousDayLookupKey] !== undefined && newEvent.datetype === 'date') {
              ...
              let exdate = event.skipTZ ? moment({ date: newEvent.exdate[previousDayLookupKey] }) : moment({ timezone, date: newEvent.exdate[previousDayLookupKey] })
              if (exdate.isSame(start, 'day')) {
      ...
      try {
        // regular event
        let start = event.skipTZ || event.start.toUTCString().includes('00:00:00') ? moment({ date: event.start }) : moment({ timezone, date: event.start })
        let end = event.skipTZ || event.start.toUTCString().includes('00:00:00') ? getRegularEventEnd({ event }) : getRegularEventEnd({ timezone, event })
