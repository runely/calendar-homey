  ...
  for (const event of Object.values(data)) {
    ...
    if (typeof event.rrule !== 'undefined') {
      ...
      dates.forEach(date => {
        ...
        let addEvent = true
        const offsetThis = moment(timezone, date.toISOString()).utcOffset() // force offset to switch from positive to negative and vice versa
        const offsetEventStart = moment(timezone, event.start.toISOString()).utcOffset() // force offset to switch from positive to negative and vice versa
        const offsetHoursToAdd = (offsetEventStart - offsetThis) / 60
        const hoursToSet = date.getHours() - offsetHoursToAdd
        // hacky wacky shitty thing that works and takes dst into account. Thanks to Mats!
        let start = moment(timezone, new Date(date.setHours(hoursToSet)))
        ...
        if (event.start.toUTCString().includes('00:00:00')) {
          start = moment(timezone, start).add(offsetEventStart, 'minutes')
        }
        ...
        const previousDay = moment(timezone, new Date(date.setHours(hoursToSet))).subtract(1, 'day')
        ...
        if (newEvent.recurrences !== undefined && newEvent.recurrences[dateLookupKey] !== undefined) {
          ...
          if (recdate.isSame(start, 'day')) {
            ...
            duration = getDuration(timezone, newEvent.start, newEvent.end)
