      ...
      try {
        ...
        if (event.recurrences !== undefined) {
          for (const recurrenceDate of Object.keys(event.recurrences)) {
            ...
            let rMomentRecurr = event.recurrences[recurrenceDate].recurrenceid.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].recurrenceid }) : moment({ date: event.recurrences[recurrenceDate].recurrenceid, timezone })
            ...
            let rMomentStart = event.recurrences[recurrenceDate].start.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].start }) : moment({ date: event.recurrences[recurrenceDate].start, timezone })
            let rDateStart = new Date(event.recurrences[recurrenceDate].start)
            ...
            if (rMomentStart.isBetween(eventLimitStart, eventLimitEnd) === true) {
              ...
              let rMomentEnd = event.recurrences[recurrenceDate].end.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].end }) : moment({ date: event.recurrences[recurrenceDate].end, timezone })
              if (rMomentStart.isBefore(eventLimitStart) && rMomentEnd.isAfter(eventLimitStart)) {
                const existAlready = dates.filter((date) => date === rDateStart)
                if (existAlready.length === 0) {
                  dates.push(rDateStart)
                  app.log(`Recurrence in '${event.summary}' (${event.uid}) is ongoing from '${rMomentRecurr}' to '${rMomentEnd}'`)
                }
              }
        ...
        localDates.forEach((date) => {
          ...
          let dateLookupKey = momentInstance(start).add(offsetThis, 'minutes').toISOString().slice(0, 10)
          ...
          let previousDay = (event.skipTZ ? momentInstance(start) : momentInstance(start).add(offsetThis, 'minutes')).subtract(1, 'day')
