          ...
          if (addEvent) {
            ...
            if (logAllEvents) {
              if (event.datetype === 'date') {
                app.log('Recurring full day event:', newEvent.summary, '--', start, '--', end, '--', newEvent.start.toUTCString(), '--', newEvent.uid)
              } else {
                app.log('Recurring event:', newEvent.summary, '--', start, '--', end, '--', newEvent.start.toUTCString(), `-- TZ:${event.skipTZ ? 'missing/invalid' : timezone}`, '--', newEvent.uid)
              }
            }
          }
        })
