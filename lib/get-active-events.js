  ...
  for (const k in data) {
    ...
    if (typeof event.rrule !== 'undefined') {
      ...
      dates.forEach(date => {
        ...
        let end = moment(Number.parseInt(start.format('x')) + duration, 'x')
        ...
        const previousDaysLookupKey = previousDay.toISOString().slice(0, 10)
        ...
        if ((newEvent.recurrences !== undefined) && (newEvent.recurrences[dateLookupKey] !== undefined)) {
          ...
          end = moment(Number.parseInt(start.format('x')) + duration, 'x')
        }
        if (end.isBefore(now) || start.isAfter(eventLimitEnd)) {
          ...
          if (newEvent.exdate[dateLookupKey] !== undefined) {
            ...
            if (exdate.isSame(start, 'day')) {
              app.log(`Skipping '${newEvent.summary}' @ `, date, '(', newEvent.exdate[dateLookupKey], ')', 'because it exists in exdate by date lookup key')
              addEvent = false
            }
          } else if (newEvent.exdate[previousDaysLookupKey] !== undefined && newEvent.datetype === 'date') {
            ...
            const exdate = moment(newEvent.exdate[previousDaysLookupKey])
            if (exdate.isSame(start, 'day')) {
              app.log(`Skipping '${newEvent.summary}' @ `, date, '(', newEvent.exdate[previousDaysLookupKey], ')', 'because it exists in exdate by previousDay lookup key')
              addEvent = false
            }
          }
        }
