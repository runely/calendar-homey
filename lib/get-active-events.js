    ...
    if (typeof event.rrule !== 'undefined') {
      try {
        ...
        if (event.recurrences !== undefined) {
          for (const recurrenceDate of Object.keys(event.recurrences)) {
            ...
            const rMomentRecurr = event.recurrences[recurrenceDate].recurrenceid.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].recurrenceid }) : moment({ date: event.recurrences[recurrenceDate].recurrenceid, timezone })
            ...
            const rMoment = event.recurrences[recurrenceDate].start.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].start }) : moment({ date: event.recurrences[recurrenceDate].start, timezone })
            const rDateStart = new Date(event.recurrences[recurrenceDate].start)
            let dateAdded = false
            ...
            if (dateAdded) app.log(`Recurrence in '${event.uid}' was moved from '${rMomentRecurr}' to ${rMoment}`)
          }
        }
        ...
        localDates.forEach(date => {
          ...
          const dateLookupKey = momentInstance(start).add(offsetThis, 'minutes').toISOString().slice(0, 10)
          ...
          const previousDay = (event.skipTZ ? momentInstance(start) : momentInstance(start).add(offsetThis, 'minutes')).subtract(1, 'day')
          ...
          if (addEvent) {
            if (!logged && logAllEvents) {
              if (event.datetype === 'date') {
                app.log('Recurring full day event:', event.summary, '--', start, '--', end, '--', event.start.toUTCString())
              } else {
                app.log('Recurring event:', event.summary, '--', start, '--', end, '--', event.start.toUTCString(), `-- TZ:${event.skipTZ ? 'missing/invalid' : timezone}`)
              }
              logged = true
            }
      ...
      try {
        ...
        if ((now.diff(end, 'seconds') < 0 && start.isBetween(eventLimitStart, eventLimitEnd) === true) || (now.diff(start, 'seconds') > 0 && now.diff(end, 'seconds') < 0)) {
          if (event.datetype === 'date') {
            app.log('Regular full day event:', event.summary, '--', start, '--', end, '--', event.start.toUTCString())
          } else if (event.datetype !== 'date' && logAllEvents) {
            app.log('Regular:', event.summary, '--', start, '--', end, '--', event.start.toUTCString(), `-- TZ:${event.skipTZ ? 'missing/invalid' : timezone}`)
          }
