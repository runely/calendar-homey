...
const removeInvalidTZ = (app, event, calendarName) => {
  ...
  try {
    if (isInvalidTZ(event.start.tz)) {
      app.log(`getActiveEvents: Invalid timezone (${event.start.tz}) found on`, event.summary, '--', event.uid)
  ...
  for (const event of Object.values(data)) {
    ...
    if (typeof event.rrule !== 'undefined') {
      try {
        ...
        app.log(`Recurring event '${event.summary}' (${event.uid}) is invalid : `, error)
      ...
      try {
        ...
        if ((now.diff(end, 'seconds') < 0 && start.isBetween(eventLimitStart, eventLimitEnd) === true) || (now.diff(start, 'seconds') > 0 && now.diff(end, 'seconds') < 0)) {
          if (logAllEvents) {
            if (event.datetype === 'date') {
              ...
              // Regular: Summary -- Start -- End -- Original Start UTC string -- TZ -- UID
              app.log('Regular:', event.summary, '--', start, '--', end, '--', event.start.toUTCString(), `-- TZ:${event.skipTZ ? 'missing/invalid' : timezone}`, '--', event.uid)
            }
          }
        ...
        app.log(`Regular event '${event.summary}' (${event.uid}) is invalid : `, error)
