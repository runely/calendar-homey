...
module.exports = data => {
    let now = moment();
    let recurrenceStart = moment().startOf('day');
    let recurrenceEnd = moment().endOf('day').add(2, 'months');
    let events = [];
    ...
    for (let k in data) {
        ...
        if (typeof event.rrule !== 'undefined') {
            // getting the set of start dates between recurrenceStart and recurrenceEnd
            //      Include dates which falls on until date: true||false
            let dates = event.rrule.between(recurrenceStart.toDate(), recurrenceEnd.toDate(), true);
            ...
            if (event.recurrences != undefined) {
                for (var r in event.recurrences) {
                    ...
                    if (moment(new Date(r)).isBetween(recurrenceStart, recurrenceEnd) != true) {
            ...
            dates.map(date => {
                ...
                if (addEvent) {
                    ...
                    if (end.isBefore(now) || start.isAfter(recurrenceEnd)) {
                        addEvent = false;
                    }
