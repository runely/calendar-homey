    ...
    for (let k in data) {
        ...
        if (typeof event.rrule !== 'undefined') {
            ...
            dates.map(date => {
                ...
                if ((event.recurrences != undefined) && (event.recurrences[dateLookupKey] != undefined)) {
                    // we found an override, so for this recurrence, use a potentially different title, start date, and duration.
                    newEvent = event.recurrences[dateLookupKey];
                ...
                else if ((event.exdate != undefined) && (event.exdate[dateLookupKey] != undefined)) {
                ...
                if (addEvent) {
                    ...
                    if (addEvent) {
                        if (newEvent) {
                            ...
                            event.start = start;
                            event.end = end;
                            event.uid = `${event.uid}_${start.toDate().toISOString().substring(0, 10)}`;
