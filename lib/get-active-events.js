      ...
      try {
        ...
        if (event.recurrences !== undefined) {
          for (const recurrenceDate of Object.keys(event.recurrences)) {
            ...
            let rMoment = event.recurrences[recurrenceDate].start.toUTCString().includes('00:00:00') ? moment({ date: event.recurrences[recurrenceDate].start }) : moment({ date: event.recurrences[recurrenceDate].start, timezone })
            let rDateStart = new Date(event.recurrences[recurrenceDate].start)
            let dateAdded = false
            if (rMoment.isBetween(eventLimitStart, eventLimitEnd) === true) {
            ...
            if (dateAdded) app.log(`Recurrence in '${event.summary}' (${event.uid}) was moved from '${rMomentRecurr}' to ${rMoment}`)
            rMomentRecurr = null
            rDateRecurr = null
            newDates = null
            rMoment = null
