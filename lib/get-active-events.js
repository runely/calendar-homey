...
module.exports = (data, eventLimit, app) => {
  ...
  for (const k in data) {
    ...
    if (!hasData(event.start) || !hasData(event.end) || !hasData(event.summary)) {
      throw new Error(`"DTSTART", "DTEND" and/or "SUMMARY" is missing in event with UID: ${event.uid}`)
    }
      ...
      dates.forEach(date => {
        ...
        if (end.isBefore(now) || start.isAfter(eventLimitEnd)) {
          ...
          if (newEvent.exdate[previousDayLookupKey] !== undefined && newEvent.datetype === 'date') {
            ...
            if (exdate.isSame(start, 'day')) {
              app.log(`Skipping '${newEvent.summary}' @ `, date, '(', newEvent.exdate[previousDayLookupKey], ')', 'because it exists in exdate by previousDay lookup key')
              addEvent = false
            }
          }
        }
