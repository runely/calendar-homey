  ...
  for (const event of Object.values(data)) {
    ...
    if (typeof event.rrule !== 'undefined') {
      try {
        ...
        const localDates = dates.map(date => moment({ timezone, date: date.toISOString() }))
        ...
        localDates.forEach(date => {
          ...
          let start = moment({ timezone, date }).add(offsetThis, 'minutes')
          let end = getEnd(start, duration, timezone)
          ...
          const previousDay = moment({ timezone, date }).subtract(1, 'day')
          ...
          if (newEvent.recurrences !== undefined && newEvent.recurrences[dateLookupKey] !== undefined) {
            ...
            const recdate = moment({ timezone, date: newEvent.recurrences[dateLookupKey].start })
            if (recdate.isSame(start, 'day')) {
              newEvent = deepClone(newEvent.recurrences[dateLookupKey])
              start = moment({ timezone, date: newEvent.start })
              duration = getDuration(newEvent.start, newEvent.end)
              end = getEnd(start, duration, timezone)
            }
          }
          ...
          if (newEvent.recurrences !== undefined && newEvent.recurrences[previousDayLookupKey] !== undefined) {
            ...
            const recdate = moment({ timezone, date: newEvent.recurrences[previousDayLookupKey].start })
            if (recdate.isSame(start, 'day')) {
          ...
          if (end.isBefore(now) || start.isAfter(eventLimitEnd)) {
            ...
            if (newEvent.exdate[dateLookupKey] !== undefined) {
              ...
              const exdate = moment({ timezone, date: newEvent.exdate[dateLookupKey] })
            ...
            if (newEvent.exdate[previousDayLookupKey] !== undefined && newEvent.datetype === 'date') {
              ...
              const exdate = moment({ timezone, date: newEvent.exdate[previousDayLookupKey] })
              if (exdate.isSame(start, 'day')) {
      ...
      try {
        // regular event
        const start = event.start.toUTCString().includes('00:00:00') ? moment({ date: event.start }) : moment({ timezone, date: event.start })
        const end = event.start.toUTCString().includes('00:00:00') ? getRegularEventEnd({ event }) : getRegularEventEnd({ timezone, event })
