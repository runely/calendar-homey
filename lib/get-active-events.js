 ...
 * @returns string
 */
const getFormattedMoment = (momentObj) => momentObj.format('DD.MM.yyyy HH:mm:ss')
...
/**
 * @param {import('node-ical').DateWithTimeZone} start
 * @param {import('node-ical').DateWithTimeZone} end
 *
 * @returns {number}
 */
...
const getEnd = (start, duration, timezone) => {
  if (timezone) {
    return moment({timezone, date: Number.parseInt(moment({date: start}).format('x'), 10) + duration, format: 'x'})
  }
...
/**
 * @param {import('../types/ExtendedHomeyApp.type').ExtHomeyApp|import('../types/AppTests.type').AppTests} app
 * @param {string} prop
 * @param {string|ValueObject} value
 * @param {string} uid
 *
 * @returns string
 */
...
/**
 * @param {import('../types/ExtendedHomeyApp.type').ExtHomeyApp|import('../types/AppTests.type').AppTests} app
 * @param {import('node-ical').VEvent} event
...
const removeInvalidTZ = (app, event, calendarName) => {
  ...
  try {
    ...
    triggerSynchronizationError({ app, calendar: calendarName, error, event })
      .catch(err => app.logError('getActiveEvents/removeInvalidTZ: Failed to complete triggerSynchronizationError(...):', err))
  }
}

/**
 * @typedef {import('../types/VariableMgmt.type').VariableManagementCalendarEvents} CalendarEvents
 */

/**
 * @typedef {object} GetActiveEventsOptions
 * @property {string} timezone - The timezone to use on events (IANA)
 * @property {import('node-ical').CalendarResponse} data - Calendar data parsed in from "node-ical"
  ...
  for (const event of Object.values(data)) {
    ...
    if (!hasData(event.start) || !hasData(event.end)) {
      triggerSynchronizationError({ app, calendar: calendarName, error: '"DTSTART" and/or "DTEND" is missing', event })
        .catch(err => app.logError('getActiveEvents: Failed to complete triggerSynchronizationError(...):', err))
    ...
    if (typeof event.rrule === 'undefined') {
      ...
      try {
        ...
        triggerSynchronizationError({ app, calendar: calendarName, error, event })
          .catch(err => app.logError('getActiveEvents/regularEvent: Failed to complete triggerSynchronizationError(...):', err))
      }
    ...
    try {
      ...
      triggerSynchronizationError({ app, calendar: calendarName, error, event })
        .catch(err => app.logError('getActiveEvents/recurringEvent: Failed to complete triggerSynchronizationError(...):', err))
    }
  }
