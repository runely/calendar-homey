        ...
        if (event.RRULE) {
            ...
            recurringDates.map(date => {
                if (event.DTSTART_TIMESTAMP) {
                    ...
                    let newEvent = { ...event, DTSTART_TIMESTAMP: startString, DTEND_TIMESTAMP: stopString, UID: `${event.UID}_${startString}` };
                    ...
                    //console.log(`get-active-events: Recurring event '${event.SUMMARY}' -- '${startString}' -- '${stopString}'`);
                    let newEvent = { ...event, DTSTART_DATE: startString, DTEND_DATE: stopString, UID: `${event.UID}_${startString}` };
                    delete newEvent.EXDATE;
                    newEvents.push(newEvent);
                }
                else {
