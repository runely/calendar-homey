...
import { GetNewEventsOptions } from "../types/Options.type";
...
export const getNewEvents = (options: GetNewEventsOptions): ExtCalendarEvent[] => {
  ...
  const newlyAddedEvents: CalendarEventUid[] = newCalendarsUids.filter((newEvent: CalendarEventUid) => !oldCalendarsUids.find((oldEvent: CalendarEventUid) => newEvent.uid === oldEvent.uid));
  ...
  newlyAddedEvents.forEach((newEvent: CalendarEventUid) => {
    const calendar: VariableManagementCalendar | undefined = calendarsEvents.find((calendar: VariableManagementCalendar) => calendar.name === newEvent.calendar);
    if (!calendar) {
      app.log(`[WARN] getNewEvents: Calendar '${newEvent.calendar}' not found 😬`);
      return;
    }

    const event: VariableManagementCalendarEvent | undefined = calendar.events.find((event: VariableManagementCalendarEvent) => event.uid === newEvent.uid);
    if (!event) {
