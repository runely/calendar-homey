...
import { VariableManagement, VariableManagementLocalEvent } from "../types/VariableMgmt.type";
import { GetLocalActiveEventsOptions } from "../types/Options.type";
...
export const getLocalActiveEvents = (options: GetLocalActiveEventsOptions): VariableManagementLocalEvent[] => {
  ...
  const eventLimitEnd: Moment = getMoment({ timezone }).endOf('day').add(Number.parseInt(eventLimit.value, 10), eventLimit.type);
  ...
  for (const event of events) {
    ...
    const start: Moment = event.skipTZ ? getMoment({ date: event.start as string }) : getMoment({ timezone, date: event.start as string });
    ...
    if ((now.diff(end, 'seconds') < 0 && start.isBetween(eventLimitStart, eventLimitEnd)) || (now.diff(start, 'seconds') > 0 && now.diff(end, 'seconds') < 0)) {
      if (logAllEvents) {
        if (event.dateType === 'date') {
          // Regular full day event: Summary -- Start -- End -- Original Start UTC string -- UID
          app.log('Local regular full day event:', event.summary, '--', start, '--', end, '--', event.start as string, '--', event.uid);
        } else {
          // Regular event: Summary -- Start -- End -- Original Start UTC string -- TZ -- UID
          app.log('Local regular event:', event.summary, '--', start, '--', end, '--', event.start as string, `-- TZ:${event.skipTZ ? 'missing/invalid' : timezone}`, '--', event.uid);
        }
      }
...
export const saveLocalEvents = (app: App, variableMgmt: VariableManagement, events: VariableManagementLocalEvent[]): void => {
  if (events.length === 0) {
